...
export default function CompanyResearchTool({
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    const progressSteps = [
      ...
      "Gathering ESG performance metrics...",
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="text-center space-y-4">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-[var(--brand)] to-[var(--brand)]/70 text-white shadow-lg">
          <Sparkles size={28} />
        </div>
        ...
        <p className="text-[var(--text-muted)] max-w-2xl mx-auto">
          Comprehensive AI-powered company intelligence for business development. 
          Get detailed insights on company profile, financials, ESG performance, credit ratings, 
          key contacts, competitive analysis, and strategic intelligence.
        </p>
      </div>
      ...
      <form onSubmit={handleSearch} className="relative">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Enter company name (e.g., Apple, Microsoft, Tesla, Saudi Aramco...)"
            className="w-full pl-12 pr-36 py-4 rounded-xl border border-[var(--border)] bg-[var(--surface)] text-[var(--text)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--brand)]/50 focus:border-[var(--brand)] transition-all text-lg"
          />
          <button
            type="submit"
            disabled={isSearching || !query.trim()}
            className="absolute right-2 top-1/2 -translate-y-1/2 px-6 py-2.5 rounded-lg bg-[var(--brand)] text-white font-medium disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90 transition-opacity flex items-center gap-2"
          >
            {isSearching ? (
              <>
      ...
      {isSearching && (
        <div className="p-6 rounded-xl bg-[var(--surface)] border border-[var(--border)] animate-pulse">
          ...
          <div className="mt-4 w-full bg-[var(--border)] rounded-full h-2 overflow-hidden">
            <div className="h-full bg-[var(--brand)] rounded-full animate-[progress_2s_ease-in-out_infinite]" 
      ...
      {result ? (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <MetadataCard
              icon={FileText}
              label="Sources"
              value={result.research_metadata.total_sources_consulted.toString()}
            />
            <MetadataCard
              icon={Clock}
              label="Duration"
              value={`${result.research_metadata.research_duration_seconds.toFixed(1)}s`}
            />
            <MetadataCard
              icon={Users}
              label="Contacts"
              value={result.contacts.length.toString()}
            />
            <MetadataCard
              icon={BarChart3}
              label="Findings"
              value={result.findings.length.toString()}
            />
          </div>

          {/* Main Report */}
          <div className="bg-[var(--surface)] rounded-2xl shadow-sm border border-[var(--border)] overflow-hidden">
            <div className="p-6 border-b border-[var(--border)] bg-gradient-to-r from-[var(--brand)]/5 to-transparent">
              <div className="flex justify-between items-start">
                ...
                {result.company_profile.website && (
                  <a 
                    href={result.company_profile.website} 
                    target="_blank" 
        ...
        /* Feature Cards - Only show when no result and not searching */
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <FeatureCard
              ...
              description="Overview, history, headquarters, founding year, and global presence"
              color="orange"
            />
            <FeatureCard
              icon={TrendingUp}
              title="Financial Health"
              description="Revenue, margins, market cap, funding history, and key ratios"
              color="blue"
            />
            <FeatureCard
              icon={Leaf}
              title="ESG Performance"
              description="MSCI, Sustainalytics, S&P, CDP scores and sustainability reports"
              color="emerald"
            />
            <FeatureCard
              icon={Shield}
              title="Credit & Risk"
              description="Credit ratings, risk factors, regulatory compliance, and controversies"
              color="red"
            />
            <FeatureCard
              icon={Users}
              title="Key Contacts"
              description="C-suite, investor relations, ESG team, and BD contacts with LinkedIn"
              color="purple"
            />
            <FeatureCard
              icon={BarChart3}
              title="Competitive Intel"
              description="Market position, competitors, M&A activity, and strategic partnerships"
              color="cyan"
            />
            <FeatureCard
              icon={Briefcase}
              title="BD Intelligence"
              description="Decision makers, budget cycles, pain points, and entry strategies"
              color="amber"
            />
            <FeatureCard
              icon={Globe}
              title="Global Operations"
              description="Office locations, subsidiaries, supply chain, and certifications"
              color="indigo"
            />
            <FeatureCard
              icon={FileText}
              title="Reports & Filings"
              description="Annual reports, SEC filings, sustainability reports with direct links"
              color="pink"
            />
          </div>
        </div>
...
function FeatureCard({
  ...
  return (
    <div className={`p-5 rounded-xl bg-gradient-to-br ${colorMap[color] || colorMap.blue} border border-[var(--border)]`}>
