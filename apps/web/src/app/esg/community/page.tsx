...
export default async function CommunityPage({
  ...
  const bgAccent = "bg-emerald-50";
  const borderAccent = "border-emerald-100";
        ...
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm font-medium text-slate-500">
                <span className="uppercase tracking-wider">ESG</span>
                <span>/</span>
                <span>Community</span>
              </div>
              ...
              <p className="text-lg text-slate-600 max-w-2xl">
                Discover what the ESG community is reading and discussing. 
          ...
          <div className="mt-8 border-t border-slate-100 pt-6">
            <div className="flex items-center gap-2">
              <Link
                href={`/esg/community?tab=articles&sort=${sort}&range=${range}&scope=${scope}`}
                className={cn(
              ...
              <Link
                href={`/esg/community?tab=digest&sort=${sort}&range=${range}&scope=${scope}`}
                className={cn(
                  "px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 flex items-center gap-2",
                  tab === "digest"
                    ? "bg-slate-900 text-white shadow-sm"
            ...
            <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex flex-wrap items-center gap-2">
                <FilterGroup label="Sort by">
                  <FilterOption 
                    href={`/esg/community?tab=articles&sort=latest&range=${range}&scope=${scope}`} 
                    active={sort === "latest"}
                    icon={Clock}
                  >
                    Latest
                  ...
                  <FilterOption 
                    href={`/esg/community?tab=articles&sort=top&range=${range}&scope=${scope}`} 
                    active={sort === "top"}
                    icon={TrendingUp}
                  >
                    Top Rated
                ...
                <FilterGroup label="Time range">
                  {(["7d", "30d", "60d"] as Range[]).map(r => (
                    <FilterOption 
                      key={r} 
                      href={`/esg/community?tab=articles&sort=${sort}&range=${r}&scope=${scope}`} 
                      active={range === r}
                    >
                      {r}
                    </FilterOption>
              ...
              <div className="flex items-center">
                <div className="bg-slate-100 p-1 rounded-lg flex items-center">
                  <Link
                    href={`/esg/community?tab=articles&sort=${sort}&range=${range}&scope=all`}
                    className={cn(
                  ...
                  <Link
                    href={`/esg/community?tab=articles&sort=${sort}&range=${range}&scope=mine`}
                    className={cn(
                      "px-3 py-1.5 text-sm font-medium rounded-md transition-all duration-200 flex items-center gap-2",
                      scope === "mine" 
                        ? "bg-white text-slate-900 shadow-sm" 
                ...
                >
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    <h3 className="text-lg font-semibold text-slate-900 mb-3 line-clamp-2 group-hover:text-blue-600 transition-colors">
                      <Link href={`/esg/articles/${a.id}?back=/esg/community`} className="focus:outline-none">
