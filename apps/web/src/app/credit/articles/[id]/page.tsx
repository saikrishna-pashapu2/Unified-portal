...
import { ArrowLeft, ExternalLink, Calendar, Building2, Heart, Users, Sparkles, TrendingUp, Award, User2, Clock, DollarSign, LineChart } from "lucide-react";
...
export default async function ArticleDetail({
  ...
  const backUrl = resolvedSearchParams.back || `/credit/articles`;

  // Use content first, fallback to summary
  const body = (item as any).content ?? (item as any).summary ?? "";
        ...
        <div className="w-[60%] h-full overflow-y-auto bg-gray-50 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 p-4">
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-4">
            ...
            <div className="border-b border-gray-100">
              {/* Hero Section */}
              <div className={`bg-gradient-to-r ${getCreditColor()} p-8 text-white rounded-t-2xl`}>
              ...
              <div className="flex items-center gap-4">
                {item.link ? (
                  <a 
                    href={item.link} 
                    target="_blank"
                    className="flex items-center gap-2 px-4 py-2 bg-slate-700 text-white rounded-xl hover:bg-slate-800 transition-colors font-medium"
                  >
...
async function Summarizer({ body }: { body: string }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
      <div className="bg-gradient-to-r from-slate-600 to-slate-500 p-6 text-white">
        <div className="flex items-center gap-3">
