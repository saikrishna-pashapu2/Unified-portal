...
export default function ViewPage() {
  ...
  useEffect(() => {
    ...
    async function loadView() {
      try {
        const res = await fetch(`/api/fitch/history/${id}/view`);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex gap-4 text-sm text-muted-foreground">
                ...
                <span>•</span>
                <span>Companies: {data?.metadata?.companies_count}</span>
                <span>•</span>
                <span className="text-green-600">Success: {data?.metadata?.success_count}</span>
                {data?.metadata?.error_count > 0 && (
                  <>
                    <span>•</span>
                    <span className="text-destructive">Errors: {data?.metadata?.error_count}</span>
                  </>
                )}
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                Uploaded: {new Date(data?.metadata?.created_at).toLocaleString()}
              </p>
            </div>
          </div>
          <a
            href={`/api/fitch/history/${id}`}
